    ...
    setInputControl: function () {
        ...
        this.mouselistenerID = cc.eventManager.addListener({
            ...
            onMouseDown: function(event) {
                if (self.node.parent.getComponent("ground").ifFirst // 当前地面是否是第一块地面
                    && !self.ifRotating // 当前棍子状态是否正在旋转
                    && !self.Player.getComponent("player").state // 人物是否正在移动
                    && !self.nextGround.getComponent("ground").ifEntering){ //下一块地面是否正在进入
                    self.speed = 10;
                }
            },
            // 鼠标释放事件
            onMouseUp: function(event) {
                if (self.node.parent.getComponent("ground").ifFirst && !self.ifRotating && !self.Player.getComponent("player").state && !self.nextGround.getComponent("ground").ifEntering){
                    self.speed = 0;
                    self.ifRotating = 1;
